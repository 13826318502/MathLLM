...
def _as_tool_response(raw: str) -> dict:
    """Turn a queued scripted reply into a function-calling response.

    Route replies become a ``route_question`` call, action replies become the
    named tool call, and anything else (``final`` or garbage) becomes a plain
    content reply with no tool calls.
    """
    try:
        data = json.loads(raw)
